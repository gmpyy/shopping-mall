    ...
    <div class="cart">
      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          ...
          <!-- 在结算页面，假如选中的商品的总数为0将无法进行结算和删除操作-->
...
export default {
  ...
  watch: {
    isEdit (value) {
      if (value) { // 是编辑状态，全部复选框都不要选中
        this.$store.commit('cart/toggleAllCheck', false)
      } else { // 不是编辑状态，全部复选框都要选中（结算状态）
  ...
  methods: {
    ...
    async handleDel () {
      if (this.selCount === 0) return // 如果没有商品被选中，无法进行删除
      await this.$store.dispatch('cart/delSelect')
      this.isEdit = false // 删除之后回到结算界面
    }
  }
}
</script>
